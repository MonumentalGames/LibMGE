...
def LoadImageUrl(url: str | list | tuple, mult: bool = False) -> Image | tuple | None:
    ...
    if isinstance(url, str):
        ...
        def __load(__num):
            __image.insert(__num, _load(url[__num]))

        for num in range(len(url)):
            if mult:
                _thread = threading.Thread(target=__load, args=(num, ))
                _thread.daemon = True
                _thread.start()
                __thread.append(_thread)
            else:
                __load(num)
...
def CreateAnimatedImage(images: list[Image] | tuple[Image]):
    _Image = Image()
    _Image._size = max(images, key=lambda img: img.size[0] * img.size[1], default=None).size
    _Image._format = ImageFormat.ARGB
    for img in images:
